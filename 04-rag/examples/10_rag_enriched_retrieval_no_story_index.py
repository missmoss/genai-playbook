...
def main() -> None:
    ...
    for index, query in enumerate(QUERIES, start=1):
        print(f"Running enriched metadata-aware query without story index {index}/{len(QUERIES)}...")
        query_embedding = embed_query(query, model=EMBEDDING_MODEL)
        candidates = cosine_search(query_embedding, chunks, top_k=30)
        analysis = analyze_query(query, model=analysis_model)
        ...
        result = {
            ...
            "story_index_used": False,
        }
        results.append(result)
        print(f"\nQuery: {query}\nRetrieved: {', '.join(result['retrieved_chunks'])}\nAnswer: {response['text']}\n")

    output_path = OUTPUT_DIR / "rag_enriched_retrieval_results.json"
    write_json(output_path, results)
    print(f"Saved enriched metadata-aware results to {output_path}")
