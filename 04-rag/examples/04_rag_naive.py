...
def build_user_prompt(chunks: list[dict[str, object]], question: str) -> str:
    passages = [f"[{chunk['chunk_id']}] {chunk['text']}" for chunk in chunks]
    return "Context passages:\n" + "\n\n".join(passages) + f"\n\nQuestion: {question}"


def main() -> None:
    ensure_output_dir()
    chunks = load_json(require_file(OUTPUT_DIR / "index_naive.json", "Run python 03_index_naive.py first."))
    model = get_env_model("RAG_GENERATION_MODEL", "gemini-2.5-flash")
    results = []

    for index, query in enumerate(QUERIES, start=1):
        print(f"Running naive RAG query {index}/{len(QUERIES)}...")
    ...
    output_path = OUTPUT_DIR / "rag_naive_results.json"
    write_json(output_path, results)
    print(f"Saved naive RAG results to {output_path}")
